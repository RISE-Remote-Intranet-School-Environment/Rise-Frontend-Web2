...
const API_DATA = [
  { name:'LOL', Year: "2BA", teacher: "HIL" },
  { name:'LLO', Year: "5MA", teacher: "MLT" },
  { name:'OLL', Year: "1BA", teacher: "J3L" },
];
...
function EnhancedTableToolbar(props: EnhancedTableToolbarProps) {
  ...
  const handleDeleteSelect = (event: React.MouseEvent<unknown>) => {
    setSelected([]);
    ...
    >
      ...
      {numSelected > 0 ? (
        <Tooltip title="Delete">
          <IconButton
              onClick={(event) => handleDeleteSelect(event)}
              >
            <DeleteIcon />
          </IconButton>
        </Tooltip>
...
export default function SyllabusPage() {
  ...
  const handleSelectAllClick = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      const newSelected = API_DATA.map((n) => n.name);
  ...
  const emptyRows =
    page > 0 ? Math.max(0, (1 + page) * rowsPerPage - API_DATA.length) : 0;
  ...
  return (  
    <Box sx={{ width: '100%' }}>
      <Paper sx={{ width: '100%', mb: 2 }}>
        ...
        displayEmpty
        >
          <MenuItem value={""}>Tout les syllabus</MenuItem>
